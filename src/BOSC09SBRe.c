...
int EvaluateCPatternSBR(Image *img, Image *SBRImg, int plane, int imgPlanes, CModel *cModel,
  CTemplate *cTemplateIntra, CTemplate *cTemplateInter, PModel *pModel,
  CPattern *cPattern, SearchArea *sa, Gray *imgGrays)
	...
	for(row = sa->ulRow ; row <= sa->lrRow ; row++)
		for(col = sa->ulCol ; col <= sa->lrCol ; col++)
			{
			if(imgGrays[GetGrayPixel(img, row, col)].codewordSize >= imgPlanes-plane)
				{
					pModelIdx = GetPModelIdx(SBRImg, row, col, plane, imgPlanes, cModel, cTemplateIntra, cTemplateInter, cPattern);
					ComputePModel(cModel, pModel, pModelIdx);
					symbol = (GetGrayPixel(SBRImg, row, col) >> plane) & 0x1;
					nNats += PModelSymbolNats(pModel, symbol);
					UpdateCModelCounter(cModel, pModelIdx, symbol);
				}
			}
...
int FindBestCPatternMode0SBR(Image *img, Image *SBRImg, int imgPlanes, int plane, CModel *cModel,
  CTemplate *cTemplateIntra, CTemplate *cTemplateInter, PModel *pModel,
  CPattern *cPattern, SearchArea *sa, Gray *imgGrays)
	...
	/* Evaluate the null pattern */
	bestNBits = EvaluateCPatternSBR(img, SBRImg, plane, imgPlanes, cModel, 
	  cTemplateIntra, cTemplateInter, pModel, workingCPattern, sa, imgGrays);
		...
		for(p = plane ; p < imgPlanes ; p++)
			...
			if(workingCPattern->size[p] < (p == plane ? cTemplateIntra->size :
			  cTemplateInter->size))
				...
				workingCPattern->pattern[p][workingCPattern->size[p]] = 1;
				workingCPattern->size[p]++;
				nBits = EvaluateCPatternSBR(img, SBRImg, plane, imgPlanes, cModel,
				  cTemplateIntra, cTemplateInter, pModel, workingCPattern, sa, imgGrays);
				if(nBits < bestNBits)
					{
					bestNBits = nBits;
...
int EncodeBitPlaneSBR(Image *img, Image *SBRImg, int imgPlanes, int plane, CModel *cModel,
  CTemplate *cTemplateIntra, CTemplate *cTemplateInter, PModel *pModel,
  CPattern *cPattern, FILE *fpOut, Gray *imgGrays)
{
	int row, col, symbol;
	...
	for(row = 0 ; row < img->nRows ; row++)
		for(col = 0 ; col < img->nCols ; col++)
		{
			/* Skip this pixel if we reach the size of the codeWord */
			if(imgGrays[GetGrayPixel(img, row, col)].codewordSize >= imgPlanes-plane)
			{
				pModelIdx = GetPModelIdx(SBRImg, row, col, plane, imgPlanes, cModel, cTemplateIntra, cTemplateInter, cPattern);
				ComputePModel(cModel, pModel, pModelIdx);
...
int main(int argc, char *argv[])
	...
	int n, p, row, col, codingPlanes, imgPlanes, plane, nBits,
	  totalBits = 0, verbosityLevel, optMode, i, origMaxIntensity;
	Image *img, *SBRImg = NULL;
	...
	if(argc == 1)
		...
		//fprintf(stderr, "Usage: Micro3DEnc [ -v (verbose) ]\n");
		...
		fprintf(stderr, "  		    [ -sbr (activate Scalable Bitplane Reduction)]\n");
		fprintf(stderr, "                    img\n");
		return 1;
		}
	...
	origMaxIntensity = img->maxIntensity;
	...
	{
		/* Array of grays */
		if(!(imgGrays = (Gray *)calloc(origMaxIntensity + 1, sizeof(Gray))))
		{
		...
		for(row = 0 ; row < img->nRows ; row++)
			for(col = 0 ; col < img->nCols ; col++)
			{
				...
				imgGrays[GetGrayPixel(img, row, col)].count++;
			}
		
		if(!(SBRImg = GetSBRImage(img, imgGrays)))
		{
			fprintf(stderr, " Error (main): Unable to perform Scalable Bitplane Reduction!\n");
			return EXIT_FAILURE;
		}
		
		SBRImg->maxIntensity = 0;
		SBRImg->maxIntensity = ImageMaxIntensity(SBRImg);
		imgPlanes = (int)(log(SBRImg->maxIntensity) / M_LN2) + 1;
		if(codingPlanes > imgPlanes)
			codingPlanes = imgPlanes;
		
		//WriteImageFile("inSBRImage.pgm", SBRImg);	
	...
	/* Maxmum intensity of input image */
	WriteNBits(origMaxIntensity-1, STORAGE_BITS_MAX_INTENSITY, fpOut);
	...
	*/
	...
	if(sbr == 'y')
	{
		/* We are using Scalable Bitplane Reduction */
		WriteNBits(1, 1, fpOut);

		/* Store an indication of which intensities are present in the image */
		//for(i = 0 ; i < imgPlanes + 1; i++)
		
		for(i = 0 ; i < origMaxIntensity + 1; i++)
		{
			/*
			if(i == 0) pModelIdx = 0;
			else pModelIdx = imgGrays[i-1].usage;
			ComputePModel(cModel, pModel, pModelIdx);
			symbol = imgGrays[i].usage;
			nNats += PModelSymbolNats(pModel, symbol);
			ArithEncodeSymbol(symbol, pModel->freqs, pModel->sum, fpOut);
			UpdateCModelCounter(cModel, pModelIdx, symbol);
			*/
	...
	*/
	...
	for(plane = imgPlanes - 1 ; plane >= imgPlanes - codingPlanes ; plane--)
	{
		switch(optMode)
		{
			case 0:
				if(sbr == 'y')
					//FindBestCPatternMode0(SBRImg, imgPlanes, plane, cModel, cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
					FindBestCPatternMode0SBR(img, SBRImg, imgPlanes, plane, cModel, cTemplateIntra, cTemplateInter, pModel, cPattern, &sa, imgGrays);
				else
			...
			case 1:
				if(sbr == 'y')
					FindBestCPatternMode1(SBRImg, imgPlanes, plane, cModel, cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
				else
			...
			case 2:
				if(sbr == 'y')
					FindBestCPatternMode2(SBRImg, imgPlanes, plane, cModel, cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
				else
		...
		//if(nBits != 0 || plane == imgPlanes - 1)
		//{
			/* Encode this bit-plane */
			if(sbr == 'y')
				nBits = EncodeBitPlaneSBR(img, SBRImg, imgPlanes, plane, cModel, cTemplateIntra, cTemplateInter, pModel, cPattern, fpOut, imgGrays);
			else
	...
	for(plane = imgPlanes - codingPlanes - 1 ; plane >= 0 ; plane--)
		{
		printf("Plane %2d: in RAW.\n", plane);	
		for(row = 0 ; row < img->nRows ; row++)
			for(col = 0 ; col < img->nCols ; col++)
			{
				
				if(sbr == 'y')
					WriteNBits(GetGrayPixel(SBRImg, row, col) >> plane, 1, fpOut);
				else
