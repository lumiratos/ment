...
int main(int argc, char *argv[])
	...
	if(argc == 1)
		{
		fprintf(stderr, "Usage: %10s [ -v (verbose) ]\n", argv[0]);
	...
	if(fpOut == NULL)
		{
		if(!(fpOut = fopen("encMicroImg09.dat", "wb")))
			{
			fprintf(stderr, "Error: can't open file '%s'\n", "encMicroImg09.dat");
			return EXIT_FAILURE;
			}
	...
	for(plane = imgPlanes - 1 ; plane >= imgPlanes - codingPlanes ; plane--)
		...
		switch(optMode)
			{
			case 0:
				FindBestCPatternMode0(img, imgPlanes, plane, cModel,
				  cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
				break;

			case 1:
				FindBestCPatternMode1(img, imgPlanes, plane, cModel,
				  cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
				break;

			case 2:
				FindBestCPatternMode2(img, imgPlanes, plane, cModel,
				  cTemplateIntra, cTemplateInter, pModel, cPattern, &sa);
				break;
		...
		nBits = EncodeBitPlane(img, imgPlanes, plane, cModel, cTemplateIntra,
		  cTemplateInter, pModel, cPattern, fpOut);

		totalBits += nBits;
		if(verbosityLevel > 0)
			{
			printf("Plane %2d: %12d bits ( %.3f bpp ) %2d: ", plane, nBits,
